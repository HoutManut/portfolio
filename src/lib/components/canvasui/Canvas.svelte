...
<script lang="ts">
  ...
  interface Props extends CanvasOptions {
    ...
    /**
     * LOCAL PATCH: sizing mode.
     *
     * "fill" is upstream: the wrapper is expected to carry its own height and
     * the content is stretched to it. On the html-in-canvas path the content
     * moves inside `<canvas layoutsubtree>` at `position: absolute; inset: 0`,
     * so a wrapper around page flow has nothing left to take height from and
     * collapses to zero.
     *
     * "flow" is for exactly that case. The content keeps `height: auto` and a
     * ResizeObserver feeds its measured height back to the wrapper as
     * `min-height`, so an intrinsically-sized block — a Typst-compiled body —
     * survives the move into the canvas. Converges in one step: the content
     * height is independent of the wrapper, not derived from it.
     */
    fit?: "fill" | "flow";
    /**
     * LOCAL PATCH: brush radius in CSS pixels, "flow" only.
     *
     * Upstream's `radius` is a fraction of the source's own height — sized for
     * "fill", where that height is the viewport. Under "flow" the height is
     * whatever the content measures to, which for a Typst-compiled body ranges
     * from a couple of short paragraphs to a long writeup: the same `radius`
     * fraction then produces a brush a few CSS pixels across on a short entry
     * and one wider than a paragraph on a long one. `radiusPx`, when set,
     * overrides `radius` with `radiusPx / flowHeight` recomputed on every
     * measurement, so the brush is a constant physical size regardless of how
     * long the prose it is drawn over happens to be. `radius` still applies
  ...
  $effect(() => {
    ...
    const measure = () => {
      flowHeight = contentEl.scrollHeight;
    };
  ...
  /* LOCAL PATCH: `radiusPx` overrides `radius` once content height is known. */
  const effectiveOptions = $derived.by(() => {
    if (fit === "flow" && radiusPx != null && flowHeight > 0) {
      return { ...options, radius: radiusPx / flowHeight };
    }
    return options;
  });
...
<!-- LOCAL PATCH: `fit` drives the wrapper min-height and the content height. -->
<div
  class={className}
  style="position: relative;{fit === 'flow' && flowHeight
    ? ` min-height: ${flowHeight}px;`
    : ''}"
>
  <canvas
    bind:this={sourceEl}
