...
<script lang="ts">
  ...
  interface Props extends RetroDitherOptions {
    class?: string;
  ...
  let instance: RetroDitherInstance | null = null;
...
<!--
  LOCAL PATCH - see canvasui/README.md. Wrapper and content elements are spans
  with display:block rather than divs, so an instance can live inside <summary>,
  whose content model is phrasing content. Layout is unchanged.
-->
<span class={className} style="position: relative; display: block;">
  <canvas
    bind:this={sourceEl}
    layoutsubtree="true"
    style={native
  ...
  >
    {#if native}
      <span
        bind:this={contentEl}
        style="position: relative; display: block; width: 100%; height: 100%; overflow: auto;"
      >
        {@render children?.()}
      </span>
    {/if}
