...
<script lang="ts">
  ...
  interface Props extends ClothOptions {
    class?: string;
    children?: import("svelte").Snippet;
    /**
     * LOCAL PATCH: sizing mode. Ported from canvasui/Canvas.svelte's own
     * `fit` patch — see that file's Props doc and README.md for the full
     * rationale.
     *
     * "fill" is upstream: the wrapper is expected to carry its own height,
     * and the content div is stretched to it with `height: 100%`. On the
     * html-in-canvas path the content moves inside `<canvas layoutsubtree>`
     * at `position: absolute; inset: 0`, so a wrapper around ordinary page
     * flow has nothing left to take height from and collapses to zero —
     * which also zeroes `cellH` in `touchImprint`'s brush-radius-to-grid
     * conversion, not just the visible fabric.
     *
     * "flow" keeps the content at `height: auto` and feeds its measured
     * height back to the wrapper as `min-height` via ResizeObserver, so an
     * intrinsically-sized block (a Typst-compiled body) survives the move
     * into the canvas. Unlike Canvas's `radius`, every Cloth size option
     * (`amplitude`, `drape`, `brushSize`, `cornerRadius`, `perspective`) is
     * already a constant CSS-pixel value rather than a fraction of the
     * source's own height, so "flow" needs no accompanying `radiusPx`-style
     * override — fixing the collapse is the whole fix.
     */
    fit?: "fill" | "flow";
  }
  ...
  const contentStyle = $derived(
    `position: relative; width: 100%; max-width: 100%; min-width: 0; height: ${contentHeight}; overflow: ${fit === "flow" ? "visible" : "hidden"};`,
  );
...
>
  ...
  >
    {#if native}
      <div bind:this={contentEl} style={contentStyle}>
        {@render children?.()}
      </div>
    {/if}
  </canvas>
