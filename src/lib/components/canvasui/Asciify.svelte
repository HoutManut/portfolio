...
export interface AsciifyOptions {
  ...
  aberration?: number;
  /**
   * LOCAL PATCH — see canvasui/README.md. Upstream always builds a text mask and
   * routes any live text through the dot-screen branch instead of the glyph ramp,
   * so real UI stays readable under the effect. Set false when the wrapped
   * content is decorative type that exists only to be asciified — protecting it
   * defeats the point. Default true, matching upstream behaviour.
   */
  protectText?: boolean;
  /**
   * LOCAL PATCH — see canvasui/README.md. Where the pointer listeners attach.
   * "wrapper" (default, upstream behaviour) tracks only over the component's own
   * box; "window" tracks the cursor anywhere on the page, which is what a fixed
   * `pointer-events: none` background layer needs in order to animate at all.
   */
  trackPointerOn?: "wrapper" | "window";
  /**
   * LOCAL PATCH — see canvasui/README.md. "auto" (default, upstream behaviour)
   * uses Chrome's html-in-canvas when present, which moves the children inside a
   * <canvas layoutsubtree> at `position: absolute; inset: 0` — fine for a bounded
   * hero, fatal for a wrapper around a whole scrolling document, which collapses
   * to zero height. "fallback" pins the component to paintFallbackSnapshot(), so
   * children stay in normal flow and every browser renders identically.
   */
  renderMode?: "auto" | "fallback";
}
...
function initializeAsciify(
  ...
  function buildTextMask() {
    if (!maskCtx) return;
    // LOCAL PATCH — see the protectText option.
    if (!config.protectText) {
  ...
  /*
   * LOCAL PATCH — see canvasui/README.md. Upstream watches documentElement for
   * class/style mutations so a theme swap re-syncs the backing colour. This site
   * has exactly one rendition (DESIGN.md forbids a prefers-color-scheme branch),
   * and AxisRail writes --detail/--mono/--casl onto documentElement.style on every
   * slider frame and toggles .tween around preset jumps. Left attached, each of
   * those ticks would queue a full DOM re-snapshot. Constructed but never
   * observed, so destroy() below stays valid.
   */
  ...
  /*
   * LOCAL PATCH — see canvasui/README.md. Upstream listens on its own wrapper,
   * which means an instance whose wrapper is `pointer-events: none` never
   * receives a pointermove and so never wakes its rAF loop — it renders one frame
   * and freezes. Ground.svelte is exactly that case: a fixed, click-through layer
   * behind the whole sheet, which must not swallow clicks but must still track
   * the cursor. `trackPointerOn: "window"` moves the listeners to the window so
   * the lens follows the cursor across the whole page. Default is unchanged.
   */
  const listenTarget: HTMLElement | Window =
    config.trackPointerOn === "window"
      ? window
...
<script lang="ts">
  ...
  onMount(() => {
    // LOCAL PATCH — see the renderMode option.
    native = options.renderMode === "fallback" ? false : supportsHtmlInCanvas();
    let disposed = false;
    (async () => {
      await tick();
      if (disposed) return;
